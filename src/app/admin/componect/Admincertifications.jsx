...
export default function AdminCertificationsPage() {
    ...
    const [formData, setFormData] = useState({ title: "", image_url: "", sort_order: 0 });
    ...
    const fetchItems = async (t) => {
        try {
            setLoading(true);
            const res = await fetch(`${API_BASE}/api/certifications`, { headers: { Authorization: `Bearer ${t}` } });
            if (res.ok) {
                const data = await res.json();
                setItems(data);
            }
    ...
    const handleOpenModal = (item = null) => {
        if (item) {
            setEditingItem(item);
            setFormData({ title: item.title, image_url: item.image_url, sort_order: item.sort_order });
        } else {
            setEditingItem(null);
            setFormData({ title: "", image_url: "", sort_order: 0 });
        }
        setIsModalOpen(true);
    };
    ...
    const handleSave = async () => {
        try {
            ...
            const url = editingItem ? `${API_BASE}/api/certifications/${editingItem.id}` : `${API_BASE}/api/certifications`;
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
                ...
                <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                    <div>
                        ...
                        <p className="text-slate-500 mt-1">จัดการรายการใบรับรองและรางวัล</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {items.map((item) => (
                            <div key={item.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col hover:shadow-md transition-shadow">
                                {/* ส่วนรูปภาพที่ล็อกขนาดเท่ากันทั้งหมด */}
                                <div className="w-full h-64 bg-white relative flex items-center justify-center border-b border-slate-100 overflow-hidden">
                                    {item.image_url ? (
                                        <img
                                        ...
                                        <div className="flex items-center justify-center h-full text-slate-300 text-sm italic">No Image</div>
                                    )}
                                    <div className="absolute top-3 right-3 bg-slate-800/80 text-white text-[10px] px-2.5 py-1 rounded-full font-bold backdrop-blur-sm">
                                        #{item.sort_order}
                                    </div>
                                </div>

                                <div className="p-5 flex-1 flex flex-col">
                                    <h3 className="font-bold text-slate-800 text-lg mb-6 line-clamp-2 h-14 flex items-center">{item.title}</h3>
                                    <div className="flex gap-2 pt-4 border-t border-slate-100">
                                        <button onClick={() => handleOpenModal(item)} className="flex-1 py-2.5 rounded-lg bg-slate-50 text-slate-600 text-sm font-bold hover:bg-indigo-50 hover:text-indigo-600 transition-colors">แก้ไข</button>
                                        <button onClick={() => handleDelete(item.id)} className="flex-1 py-2.5 rounded-lg bg-red-50 text-red-600 text-sm font-bold hover:bg-red-100 transition-colors">ลบ</button>
                                    </div>
                                </div>
                            </div>
                        ))}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden">
                        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                            <h3 className="font-bold text-lg text-slate-800">{editingItem ? "แก้ไขใบรับรอง" : "เพิ่มใบรับรองใหม่"}</h3>
                            <button onClick={() => setIsModalOpen(false)} className="text-slate-400 hover:text-slate-600">✕</button>
                        </div>
                        <div className="p-6 space-y-5">
                            <InputField label="ชื่อใบรับรอง / รางวัล" value={formData.title} onChange={(v) => setFormData({ ...formData, title: v })} placeholder="เช่น ISO 9001:2015" />
                            ...
                            <ImageUploadField label="รูปภาพใบรับรอง" token={token} value={formData.image_url} onChange={(url) => setFormData({ ...formData, image_url: url })} />
                        </div>
