...
export default function AdminPartnerLogosPage() {
    ...
    // เพิ่ม field is_active เข้ามาด้วย
    const [formData, setFormData] = useState({ name: "", image_url: "", sort_order: 0, is_active: true });
    ...
    const fetchItems = async (t) => {
        try {
            setLoading(true);
            // ใช้ endpoint partner-logos
            const res = await fetch(`${API_BASE}/api/partner-logos`, { headers: { Authorization: `Bearer ${t}` } });
            if (res.ok) {
                const data = await res.json();
                setItems(data);
            }
    ...
    const handleOpenModal = (item = null) => {
        if (item) {
            ...
            setEditingItem(null);
            setFormData({ name: "", image_url: "", sort_order: 0, is_active: true });
        }
        setIsModalOpen(true);
    };
    ...
    const handleSave = async () => {
        try {
            ...
            // ใช้ endpoint partner-logos
            const url = editingItem ? `${API_BASE}/api/partner-logos/${editingItem.id}` : `${API_BASE}/api/partner-logos`;
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const res = await fetch(`${API_BASE}/api/partner-logos/${id}`, {
                method: "DELETE",
                headers: { Authorization: `Bearer ${token}` },
            });
            if (res.ok) {
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
                <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                    <div>
                        ...
                        <h1 className="text-3xl font-bold text-slate-900">Partner Logos</h1>
                        <p className="text-slate-500 mt-1">จัดการโลโก้พาร์ทเนอร์และแบรนด์คู่ค้า</p>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                        {items.map((item) => (
                            <div key={item.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col hover:shadow-md transition-shadow group relative">
                                {/* Active Status Badge */}
                                <div className={`absolute top-2 left-2 z-10 w-2 h-2 rounded-full ${item.is_active ? 'bg-emerald-500' : 'bg-slate-300'}`} title={item.is_active ? "Active" : "Inactive"}></div>
                                ...
                                <div className="aspect-square bg-white relative border-b border-slate-100 flex items-center justify-center p-6">
                                    {item.image_url ? (
                                        <img
                                            ...
                                            alt={item.name}
                                            className="max-w-full max-h-full object-contain transition-all duration-300 grayscale group-hover:grayscale-0"
                                        />
                                    ) : (
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden">
                        ...
                        <div className="p-6 space-y-5">
                            <InputField label="ชื่อพาร์ทเนอร์ / แบรนด์" value={formData.name} onChange={(v) => setFormData({ ...formData, name: v })} placeholder="เช่น Acer, Microsoft" />
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <label className="block space-y-1.5">
                                    ...
                                    <select
                                        ...
                                        className="w-full rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-slate-900 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                                    >
