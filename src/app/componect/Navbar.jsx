                                                ...
                                                <div className="space-y-1 max-h-[60vh] overflow-y-auto custom-scrollbar">
                                                    {loading ? (
                                                        ...
                                                        item.data.map((cat) => (
                                                            <button
                                                                key={cat.id}
                                                                onClick={() => goCategory(item.key, cat.slug || cat.id || cat.title)}
                    ...
                    {menuConfig.map((item, i) => (
                        <div key={i} className="border-b border-white/5 pb-4">
                            {item.type === "dropdown" ? (
                                <>
                                    ...
                                    >
                                        <div className="flex flex-col gap-4 pl-6 border-l-2 border-[#DAA520]/40">
                                            <button
                                                onClick={() => goCategory(item.key, "all")}
                                                className="text-left text-[#DAA520] font-black text-[10px] uppercase tracking-widest"
                                            >
                                                ดูทั้งหมด
                                            </button>
                                            {item.data.map((cat) => (
                                                <button
                                                    key={cat.id}
                                                    onClick={() => goCategory(item.key, cat.slug || cat.id || cat.title)}
                                                    className="text-left text-zinc-400 text-base font-bold hover:text-white transition-colors"
                                                >
                                                    {cat.title}
                                                </button>
                                            ))}
                                ...
                                <button
                                    onClick={() => goHash(item.href)}
                                    className="block w-full text-left text-xl font-black text-white tracking-tight"
                                >
                                    {item.label}
                                </button>
                            )}
